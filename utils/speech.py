...
def _record(if_cmu: bool = False):
    ...
    if not path.exists(path.join(path.dirname(path.abspath(__file__)), "audio")):
        os.mkdir(path.join(path.dirname(path.abspath(__file__)), "audio"))
...
def speech_to_text_baidu(audio_path: str = "test.wav", if_microphone: bool = True):
    ...
    app_id = "41667436"
    api_key = "naamn0koSG3TUUHs5SIgVkPf"
    secret_key = "NtY4Yas6CYUQi5I3yS8GyWv8Yw7N6VGQ"
    client = AipSpeech(app_id, api_key, secret_key)

    # 麦克风读入
    if if_microphone:
        result = client.asr(_record(), 'pcm', 16000, {
            'dev_pid': 1537,  # 1537识别普通话，使用输入法模型
        })
    # 文件读入
    else:
